...
void digHandler() {
  ...
  REG(GPIO_STATUS_W1TC_REG)[0]=0xFFFFFFFF;
   ...
   //dmabuff[0]=0;
   printf("fifo%08x dma%08x\n",(int)REG(I2S_FIFO_RD_REG)[0],(int)dmabuff[i%300]);//(int)REG(I2S_INT_RAW_REG)[0]);
   i++;
...
for (uint16_t j=0; j<200;j++) dmabuff[j]=j+1;
  ...
  if (true) {
   ...
   digHandler();
     ...
     CHANG(I2S_INT_CLR_REG,0xFFFFFFFF)
     prr("I2S_INT_RAW_REG just now",I2S_INT_RAW_REG);
      spin(100000);
   digHandler();
      printR();
  //rtcHandler();
